...
exports.run = async (client, message, args) => {
    ...
    if (amount === 999999999999999999999999999999999999) return message.channel.send("Balance Unlimiteed.");

    await db.add(`account.${user.id}.balance`, amount);
    await db.subtract(`account.${message.author.id}.balance`, amount);

    return message.channel.send(`You've been transferred to your friends (${user.tag}) :dollar:$${amount} credits!`);
}
